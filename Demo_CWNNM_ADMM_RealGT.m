    ...
    for lambda = [8:2:12]
        ...
        for i = 1:im_num
            ...
            for c = 1:ch
                ...
                Par.nSig0(c) = NoiseEstimation(Par.nim(:, :, c), Par.ps);
            end
            %                 NoiseEstimation(Par.nim, Par.ps)
            ...
            time0 = clock;
            [im_out, Par] = CWNNM_ADMM2_Denoising( Par.nim, Par.I, Par ); % WNNM denoisng function
        ...
        name = sprintf([Par.method '_CC' num2str(im_num) '_' Par.model '_Oite' num2str(Par.Iter) '_Iite' num2str(Par.maxIter) '_rho' num2str(rho) '_mu' num2str(Par.mu)  '_lambda' num2str(lambda) '.mat']);
