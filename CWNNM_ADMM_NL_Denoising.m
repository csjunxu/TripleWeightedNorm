function [E_Img, Par]   =  CWNNM_ADMM_NL_Denoising( N_Img, O_Img, Par )
...
for iter = 1 : Par.Iter
    ...
    for c = 1:Par.ch
        if(iter == 1)
            ...
            TempSigma_arrCh = Par.lambda(Par.iter) * Sigma(c, :);
        end
    ...
    [Y_hat, W_hat, Sigma]  =  CWNNM_ADMM_NL_Estimation( NL_mat, Sigma_arrCh, NoiPat, Par );   % Estimate all the patches
