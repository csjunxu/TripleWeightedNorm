...
%     CurPatCh = [CurPat(1:Par.ps2, :) CurPat(Par.ps2+1:2*Par.ps2, :) CurPat(2*Par.ps2+1:3*Par.ps2, :)];
    ...
    for c = 1:Par.ch
        Sigma_arrCh(c, :) = Par.lambda*sqrt(abs(repmat(Par.nSig(c)^2, 1, size(CurPat, 2)) - mean((NoiPat((c-1)*Par.ps2+1:c*Par.ps2, :) - CurPat((c-1)*Par.ps2+1:c*Par.ps2, :)).^2)));
    end
    if (mod(iter-1, Par.Innerloop) == 0)
        ...
        if(iter == 1)
            Sigma_arrCh = bsxfun(@times, Par.nSig' , ones(Par.ch, Par.TolN));                       % First Iteration use the input noise parameter
        end
    end
    % Inexact ALM for Weighted WNNM
    [Y_hat, W_hat]  =  WALMPatEstimation( NL_mat, Sigma_arrCh, CurPat, Par );   % Estimate all the patches
